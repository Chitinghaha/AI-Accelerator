...
bool myGemm_Kernel()
{
    ...
    { /******************      8-2.1-4 : Quantization      ******************/
        ...
        for (int m = 0; m < GEMM_dimM; m++)
        {
            ...
            {
                gemm_b[m * GEMM_dimN + n] = (int8_t)(rand() % (2 * _8_BIT_) - _8_BIT_);
                for (int k = 0; k < GEMM_dimK; k++)
                {
                    gemm_w[k * GEMM_dimN + n] = (int8_t)(rand() % (2 * _8_BIT_) - _8_BIT_);
                    data[m * GEMM_dimK + k] = (int8_t)(rand() % (2 * _8_BIT_) - _8_BIT_);
                }
                gemm_b[m * dimN + n] = (int8_t)(rand() % (2 * _8_BIT_) - _8_BIT_);
            }
        }
        ...
        for (int m = 0; m < GEMM_dimM; m++)
            for (int n = 0; n < GEMM_dimN; n++)
                correct += ((gemm[m * GEMM_dimN + n]) == (gemmTest[m * GEMM_dimN + n]) ? 1 : 0);
        gemm_cnt += (correct == GEMM_dimM * GEMM_dimN ? 1 : 0);
    }
    printf("[ TEST ] `GEMM`  :                       %3d/%3d\n", gemm_cnt, TB_SIZE);

    return gemm_cnt == TB_SIZE ? true : false;
}